...
void CheckCountsTrigger(bool isemulator=true){
  ...
  const int isogammasteps=6;
  const int tausteps=6;
  const int isotausteps=6;
  
  int isjetsabove[jetsteps]={0,0,0,0,0,0};
  int nthreshgammas[egammasteps]={0,0,0,0,0,0};
  int nthreshisoegammas[isogammasteps]={0,0,0,0,0,0};
  int nthreshtaus[tausteps]={0,0,0,0,0,0};
  int nthreshisotaus[isotausteps]={0,0,0,0,0,0};

  double energyjetthres[jetsteps]={16.,32.,52.,68.,92.,128.};
  double energygammathres[egammasteps]={5.,10.,20.,25.,30.,35.,40.};
  double energyisogammathres[isogammasteps]={18.,20.,22.,25.,28.,30};
  ...
  for(Long64_t j = 0; j < l_entries; ++j){
    ...
    int counter_isoegammasabove[isogammasteps]={0,0,0,0,0,0};
    ...
    for (int i=0;i<Njets;i++){
      ...
      for(int m=0;m<egammasteps;m++) {if((egamma_hwPt[i]*4)>energygammathres[m])  counter_egammasabove[m]++;}
      for(int m=0;m<isogammasteps;m++) {if((egamma_hwPt[i]*4 && egamma_hwIso[i]==1)>energyisogammathres[m])  counter_isoegammasabove[m]++;}
    }
